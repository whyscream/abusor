    ...
    try:
        if ip_address.version == 4:
            if not GEOIP_IPV4_CACHE:
                GEOIP_IPV4_CACHE = GeoIP.open(
                    GEOIP_IPV4_COUNTRY_DATABASE, GeoIP.GEOIP_STANDARD
                )
            country_code = GEOIP_IPV4_CACHE.country_code_by_addr(ip_address.compressed)
        elif ip_address.version == 6:
            if not GEOIP_IPV6_CACHE:
                GEOIP_IPV6_CACHE = GeoIP.open(
                    GEOIP_IPV6_COUNTRY_DATABASE, GeoIP.GEOIP_STANDARD
                )
            country_code = GEOIP_IPV6_CACHE.country_code_by_addr_v6(
